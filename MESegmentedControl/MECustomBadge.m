...
+ (MECustomBadge *)customBadgeWithString:(NSString *)badgeString withStringColor:(UIColor *)stringColor withInsetColor:(UIColor *)insetColor withBadgeFrame:(BOOL)badgeFrameYesNo withBadgeFrameColor:(UIColor *)frameColor withScale:(CGFloat)scale withShining:(BOOL)shining {
...
- (void)drawRoundedRectWithContext:(CGContextRef)context withRect:(CGRect)rect {
  ...
  if (!CGSizeEqualToSize(self.shadowOffset, CGSizeZero))
    CGContextSetShadowWithColor(context, self.shadowOffset, 3, [[UIColor blackColor] CGColor]);
